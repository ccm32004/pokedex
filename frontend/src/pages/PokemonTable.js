...
const PokemonTable = () => {
  ...
  useEffect(() => {
    const fetchPokemons = async () => {
      try {
        ...
        const pokemonsWithTotal = response.data.data.pokemons.map(
          (pokemon) => ({
            ...pokemon,
            base: {
              ...pokemon.base,
  ...
  const filteredData = useMemo(() => {
    return data.filter((pokemon) => {
      const matchesName = pokemon.name.english
        .toLowerCase()
              ...
              <TableRow>
                {columns.map((column) => (
                  <TableCell
                    ...
                    sx={{
                      backgroundColor:
                        orderBy === column.id ? "primary.light" : "inherit",
                    }}
                  >
                    <TableSortLabel
                      ...
                      sx={{
                        color:
                          orderBy === column.id ? "primary.main" : "inherit",
                ...
                <TableRow key={row.id} className="table-row-hover">
                  <TableCell size="small">
                    <div className="pokemon-image-container">
                      <img
                        src={`/sprites/${String(row.id).padStart(
                          3,
                          "0"
                        )}MS.png`}
                        alt={`Pokémon ${row.id}`}
                  ...
                  <TableCell>
                    <Link to={`/pokemon/${row.id}`} className="link">
                      {row.name.english}
                    </Link>
                  </TableCell>
